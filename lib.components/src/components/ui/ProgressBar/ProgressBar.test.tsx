...
describe('ProgressBar', () => {
  ...
  it('shows percentage when enabled', () => {
    renderWithTheme(<ProgressBar value={45} showPercentage data-testid='progress-bar' />);

    expect(screen.getByText('45%')).toBeInTheDocument();
  });

  it('shows value when enabled', () => {
    renderWithTheme(<ProgressBar value={7} max={10} showValue data-testid='progress-bar' />);
  ...
  it('prioritizes percentage over value when both enabled', () => {
    renderWithTheme(
      <ProgressBar value={60} max={100} showValue showPercentage data-testid='progress-bar' />
    );
  ...
  it('renders indeterminate progress', () => {
    renderWithTheme(<ProgressBar value={50} indeterminate data-testid='progress-bar' />);
  ...
  it('renders striped progress bar', () => {
    renderWithTheme(<ProgressBar value={50} striped data-testid='progress-bar' />);

    expect(screen.getByTestId('progress-bar')).toBeInTheDocument();
  });

  it('renders animated striped progress bar', () => {
    renderWithTheme(<ProgressBar value={50} striped animated data-testid='progress-bar' />);

    expect(screen.getByTestId('progress-bar')).toBeInTheDocument();
  });

  it('handles zero value correctly', () => {
    renderWithTheme(<ProgressBar value={0} showPercentage data-testid='progress-bar' />);
  ...
  it('has proper accessibility attributes', () => {
    renderWithTheme(<ProgressBar value={50} label='File upload' data-testid='progress-bar' />);
  ...
  it('renders with custom className', () => {
    renderWithTheme(
      <ProgressBar value={50} className='custom-progress' data-testid='progress-bar' />
    );
  ...
  it('handles fractional values correctly', () => {
    renderWithTheme(<ProgressBar value={33.33} showPercentage data-testid='progress-bar' />);

    expect(screen.getByText('33%')).toBeInTheDocument();
  });

  it('works with custom max value', () => {
    renderWithTheme(<ProgressBar value={15} max={20} showValue data-testid='progress-bar' />);
