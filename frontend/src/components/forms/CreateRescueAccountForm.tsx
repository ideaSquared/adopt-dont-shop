...
interface CreateRescueAccountFormProps {
  onFirstNameChange: (value: string) => void;
  onLastNameChange: (value: string) => void;
  onEmailChange: (value: string) => void;
  onPasswordChange: (value: string) => void;
  onConfirmPasswordChange: (value: string) => void;
  onRescueTypeChange: (value: string) => void;
  rescueType: string;
  onRescueNameChange: (value: string) => void;
...
const CreateRescueAccountForm: React.FC<CreateRescueAccountFormProps> = ({
  onFirstNameChange,
  onLastNameChange,
  onEmailChange,
  onPasswordChange,
  ...
  const selectRescueType = (type: string) => {
    setSelectedType(type);
    onRescueTypeChange(type);
  };

  const isSelected = (type: string) => selectedType === type ? 'bg-cyan-300 text-white' : 'bg-white text-gray-700';
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-4 text-center'>
        {['individual', 'charity', 'company', 'other'].map((type) => (
          <button
            key={type}
            type='button'
            className={`block w-full h-full border border-gray-300 rounded-md p-4 ${isSelected(type)}`}
