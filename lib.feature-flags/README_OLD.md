...
import { feature-flagsService } from '@adopt-dont-shop/lib-feature-flags';

// Basic usage
const result = await feature-flagsService.exampleMethod({ test: 'data' });
console.log(result);
...
const customService = new FeatureFlagsService(config);
...
- `options` (FeatureFlagsServiceOptions): Operation options
...
export { feature-flagsService } from '@adopt-dont-shop/lib-feature-flags';

// In your component
import { feature-flagsService } from '@/services';
...
function MyComponent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await feature-flagsService.exampleMethod({ 
...
app.get('/api/feature-flags/example', async (req, res) => {
  try {
    const result = await featureFlagsService.exampleMethod(req.body);
    res.json(result);
  } catch (error) {
...
```bash
# From workspace root
docker-compose -f docker-compose.lib.yml up lib-feature-flags
```

2. **Run tests:**
```bash
docker-compose -f docker-compose.lib.yml run lib-feature-flags-test
```
...
COPY lib.feature-flags /workspace/lib.feature-flags

# Install dependencies
RUN npm install @adopt-dont-shop/lib-feature-flags@workspace:*
```
...
# Install dependencies
RUN cd lib.feature-flags && npm ci && npm run build
...
├── src/
│   ├── services/
│   │   ├── feature-flags-service.ts     # Main service implementation
│   │   └── __tests__/
│   │       └── feature-flags-service.test.ts
│   ├── types/
...
feature-flagsService.updateConfig({
...
```typescript
import { feature-flagsService, ErrorResponse } from '@adopt-dont-shop/lib-feature-flags';

try {
  const result = await feature-flagsService.exampleMethod(data);
  // Handle success
} catch (error) {
...
```typescript
feature-flagsService.updateConfig({ debug: true });
```
