...
A `.gitmessage` template at the repo root prefills `git commit` with the conventional-commit format and a list of allowed types/scopes. `pnpm bootstrap` wires it in automatically via `git config commit.template .gitmessage`; if you skipped bootstrap, run that command yourself to enable it.
