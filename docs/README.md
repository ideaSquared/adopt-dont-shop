...
- [Environment variable reference](./env-reference.md) — every non-essential env var, grouped by domain (the essentials live in [`.env.example`](../.env.example))
...
- [.github CI/CD overview](../.github/CI-CD.md) — GitHub configuration and workflows overview
...
- [ADR 0011 — interim availability posture](./adr/0011-interim-availability-posture.md) — _accepted_: single-host SPOF budget (24h RPO / ≤2h DB-RTO) accepted for the eyes-on launch; auto-rollback-on-failed-health **implemented**, the rest of the HA build deferred behind promotion triggers (ADS-1252, builds on ADR 0009)
...
- [ADR 0012 — internal defence-in-depth posture](./adr/0012-internal-defence-in-depth.md) — _accepted_: mTLS acceptance re-affirmed (→ multi-host), gateway auth backstop **implemented** (protected-by-default), migration back-compat deferred to ADR 0008 (ADS-1255)
...
- [Frontend recommendations plan](./frontend/recommendations-plan.md) — _superseded_: pre-implementation roadmap for the matching/recommendation system (see matching system scope above)
