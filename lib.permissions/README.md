...
// Basic usage
const result = await permissionsService.exampleMethod({ test: 'data' });
console.log(result);

// Or create a custom instance
const config: PermissionsServiceConfig = {
  apiUrl: 'https://api.example.com',
  debug: true,
};

const customService = new PermissionsService(config);
...
```typescript
new PermissionsService(config?: PermissionsServiceConfig)
```
...
- `options` (PermissionsServiceOptions): Operation options
...
function MyComponent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await permissionsService.exampleMethod({ 
...
app.get('/api/permissions/example', async (req, res) => {
  try {
    const result = await permissionsService.exampleMethod(req.body);
    res.json(result);
  } catch (error) {
...
```bash
# From workspace root
docker-compose -f docker-compose.lib.yml up lib-permissions
```

2. **Run tests:**
```bash
docker-compose -f docker-compose.lib.yml run lib-permissions-test
```
...
COPY lib.permissions /workspace/lib.permissions

# Install dependencies
RUN npm install @adopt-dont-shop/lib-permissions@workspace:*
```
...
# Install dependencies
RUN cd lib.permissions && npm ci && npm run build
...
├── src/
│   ├── services/
│   │   ├── permissions-service.ts     # Main service implementation
│   │   └── __tests__/
│   │       └── permissions-service.test.ts
│   ├── types/
...
permissionsService.updateConfig({
...
```typescript
import { permissionsService, ErrorResponse } from '@adopt-dont-shop/lib-permissions';

try {
  const result = await permissionsService.exampleMethod(data);
  // Handle success
} catch (error) {
...
```typescript
permissionsService.updateConfig({ debug: true });
```
