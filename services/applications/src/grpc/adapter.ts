// Adapter — wraps the plain async handler functions (Phase 5.3b)
// in the grpc-js `(call, callback)` signature ts-proto generates.
//
// All ApplicationService RPCs require a principal — there's no
// token-minting flow on this surface (auth handles that). Even
// StartDraft requires a principal because drafts are user-scoped.
// CONCURRENCY (from optimistic-concurrency UNIQUE constraints on
// application_events.aggregate_id+version) surfaces as a HandlerError
// with code INTERNAL and a message; the gateway translates that to a
// 409 in Phase 5.3d.
//
