// Cross-instance scheduled-run claim. Ported from
// services/notifications/src/scheduler/claim.ts (ADS-1052) for the
// application-drafts retention purge job (ADS-1320).
//
// The scheduler runs per-replica with an in-memory nextRunAt, so with >1
// replica each would independently run a scheduled job — e.g. duplicate
// purge passes. `claimScheduledRun` makes a given (job, scheduled-slot) run
