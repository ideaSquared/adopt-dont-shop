// Adapter — wraps the plain async handler functions (Phase 8.3b)
// in the grpc-js `(call, callback)` signature ts-proto generates.
//
// All 14 ModerationService RPCs require a principal — moderators /
// admins for the action surface (AssignReport, ResolveReport,
// LogModeratorAction, IssueSanction), and any authenticated user
// for FileReport / OpenSupportTicket. No anonymous surface, so the
// single `adapt` variant covers the entire RPC set.
//
