...
| `deploy.yml`               | Manual deploy to staging or production via GHCR + SSH. Ships compose/nginx/observability config on every run, gates on per-service `/health/ready`, and fails a production run if observability is off (ADS-1307/1308/1312).                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `backup.yml`               | Nightly Postgres + uploads + NATS JetStream snapshot to S3 (ADS-811 / ADS-1239 / ADS-1325). Runs in the `backups` GitHub environment (ADS-1305) — **the maintainer must create this environment (Settings → Environments → New environment → `backups`, no required reviewers) and copy the `HETZNER_HOST` / `HETZNER_SSH_KEY` / `HETZNER_HOST_FINGERPRINT` secrets into it**; a schedule-triggered job entering a reviewer-gated environment like `production` would wait for approval and expire.                                                                                                                                                                 |
| `backup-restore-drill.yml` | Automated restore-verification drill (Postgres + uploads) against the latest backup (ADS-1240 / ADS-1325 / ADR 0007). Also runs in the `backups` environment (ADS-1305) so it reads the same secrets — **the maintainer must additionally copy the `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` read-only S3 secrets into `backups`** (this job runs on a GitHub-hosted runner with no host to borrow the SSH-based credentials from). Both this workflow and `backup.yml` post to an optional `DISCORD_WEBHOOK_URL` secret and open/update a `backup-failure`-labelled GitHub issue on failure (ADS-1324) — `DISCORD_WEBHOOK_URL` is skipped gracefully if unset. |
...
The `ci-required` aggregator job fans in thirteen jobs (`workspace-drift`, `commit-lint`, `changes`, `build-libs`, `test-frontend`, `test-libs`, `test-packages`, `test-services`, `test-contracts`, `dev-auth-guard`, `test-e2e`, `test-e2e-smoke`, `validate-observability-config`) and is the single required status check for branch protection — see [Branch Protection](#branch-protection). The per-job breakdown lives in [CONTRIBUTING.md → Full CI matrix](../../CONTRIBUTING.md#before-opening-a-pr).

**E2E strategy** (ADS-386 / ADS-419, smoke gate ADS-1242 / ADS-1321):

The full Playwright suite (`test-e2e`) is **opt-in on pull requests** — the
full docker-stack build plus Playwright run is too slow to pay on every push
during development. It runs when:
...
treats as success — so the full suite never blocks an in-progress PR. In its
place, the `test-e2e-smoke` job runs the `@smoke` critical-path subset
automatically on PRs touching app/service code (unless labelled `run-e2e`,
since the full suite already covers those journeys). **Unlike the full
suite, `test-e2e-smoke` IS in `ci-required`'s needs (ADS-1321 / P1-15)** — a
smoke failure blocks merge, closing the gap where any of the headline
...
- **Pull request and push to main/develop** (ADS-1314): both trigger on the same broad source path set (`apps/**`, `packages/**`, `services/**`, plus Dockerfiles/compose/`.dockerignore`) — a source-only PR used to be covered only by `ci.yml`'s `test-e2e`, but that brings up **development**-target images, so a regression that only manifests in the **production** target (e.g. a config-validation path gated on `NODE_ENV=production`) had no pre-merge signal. Both events now build development images (all three apps, all eleven services) and production images + the Trivy vulnerability scan (services always; apps' production build stays push-only — `ci.yml`'s `test-frontend` matrix already exercises the equivalent `tsc && vite build`, and Trivy already scans the service images every trigger).
- **`prod-image-smoke`** (ADS-1314 / P1-8, **required check**): the only job that actually boots a production image. Builds the `production` target for the gateway, `service-auth`, `service-notifications`, and `app-client` with `load: true` (never pushed), starts them against real Postgres/Redis/NATS/ClamAV via `docker-compose.prod.yml` + the CI-only `docker-compose.ci-smoke.yml` override (throwaway file secrets + the `EMAIL_PROVIDER`/`PUSH_PROVIDER`/`LEGAL_DOCS_DIR` env the base compose doesn't set), and asserts `/health/ready` on each plus `GET /api/v1/legal/terms` on the gateway. Exists because production images were built and CVE-scanned but never booted — the exact artifact `deploy.yml` ships had never been health-checked before landing on `main`.

The previous `test-compose` job (a container `/health` probe) was removed; `ci.yml`'s `test-e2e` brings up the full (development-target) stack and `prod-image-smoke` above now covers the production-target gap that left.
...
Four required status checks on `main`:

| Check                                  | Source workflow          | Why required                                                                                                                                                                                                                                                                      |
| -------------------------------------- | ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CI Required`                          | `ci.yml`                 | Aggregates workspace-drift, commit-lint, build-libs, backend/frontend/library/package tests (with coverage gates), contract tests, dev-auth-guard, E2E, **E2E Smoke** (ADS-1321), and observability-config validation (ADS-1324).                                                 |
...
| `Production Image Smoke`               | `docker.yml`             | ADS-1314: boots the `production`-target gateway/auth/notifications/app-client images against real Postgres/Redis/NATS and asserts `/health/ready` + a real gateway route — the artifact `deploy.yml` actually ships was previously never health-checked before landing on `main`. |

`strict_required_status_checks_policy` is `true` (ADS-1316): a PR's required
checks must have run against the branch's current base, not a stale one, or
GitHub asks for an update/re-run before merge.
...
- **E2E (Playwright)** — opt-in integration signal: runs on main pushes, on PRs
  ...
  breakage.
- **E2E Smoke (ADS-1321 / P1-15)** — the `@smoke` critical-path subset now
  blocks merge too (it was advisory-only before): any of the headline
  journeys (auth, adoption application, chat, gateway login) regressing could
  previously merge green. It runs on the same PRs the full suite would skip,
  so between the two, every merged PR gets at least the smoke signal.
- **Production Image Smoke (ADS-1314 / P1-8)** — the only check that boots the
  actual `production`-target images (E2E only exercises `development`-target
  images); catches config-validation-at-boot and missing-file-in-image classes
  of bug that a dev-target stack can't surface.
