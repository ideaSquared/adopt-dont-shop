...
describe('AuthController - Two-Factor Authentication', () => {
  ...
  describe('twoFactorSetup', () => {
    it('should return secret and QR code data URL for a user without 2FA enabled', async () => {
      ...
      MockedAuthService.generateQrCodeDataUrl = vi
        .fn()
  ...
  describe('twoFactorEnable', () => {
    it('should enable 2FA and return backup codes when setup token is valid', async () => {
      ...
      expect(MockedAuthService.enableTwoFactor).toHaveBeenCalledWith(
        'user-123',
        'JBSWY3DPEHPK3PXP',
        '123456'
      );
    ...
    it('should return 400 when setup token is invalid', async () => {
      ...
      MockedAuthService.enableTwoFactor = vi
        .fn()
        .mockRejectedValue(new Error('Invalid verification code. Please try again.'));
  ...
  describe('twoFactorDisable', () => {
    it('should disable 2FA when valid TOTP token provided', async () => {
      ...
      const userWithSecret = createMockDbUser({
        twoFactorEnabled: true,
        twoFactorSecret: 'JBSWY3DPEHPK3PXP',
      });
      MockedUser.scope = vi
        .fn()
        .mockReturnValue({ findByPk: vi.fn().mockResolvedValue(userWithSecret) });
      ...
      expect(MockedAuthService.verifyTwoFactorSetupToken).toHaveBeenCalledWith(
        'JBSWY3DPEHPK3PXP',
        '123456'
      );
