...
| `resend_api_key`           | `service-notifications`                                     | Resend API key (`re_...`) — required because `EMAIL_PROVIDER=resend` in staging/production (ADS-1302). Not generated by `pnpm secrets:generate`; it's a real third-party credential from your Resend account, not a random value.                    |
| `fcm_service_account_json` | `service-notifications`                                     | Full Firebase service-account JSON key file contents — required because `PUSH_PROVIDER=fcm` in staging/production (ADS-1302). Not generated by `pnpm secrets:generate`; download it from the Firebase console (Project settings → Service accounts). |

Two more env vars ride alongside the `service-notifications` secrets above but
aren't secrets themselves (set directly in the compose file / host `.env`,
not as `./secrets/*` files): `FCM_PROJECT_ID` (your Firebase project ID) and
`DEFAULT_FROM_EMAIL` (the Resend-verified sending address).
