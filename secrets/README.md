...
| `nats_auth_token`       | `nats` container + `service-gateway` + every domain service | Shared token required by the `nats` container's `--auth` flag; every publisher/subscriber must present it to connect (ADS-1273)                                             |
