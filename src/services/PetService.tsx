...
const PetService = {
  fetchPets: async (rescueId: string): Promise<Pet[]> => {
    try {
      const response: AxiosResponse<Pet[]> = await axios.get(`${API_BASE_URL}/pets/owner/${rescueId}`, {
  ...
  fetchAllPets: async (): Promise<Pet[]> => {
    try {
      const response: AxiosResponse<Pet[]> = await axios.get(`${API_BASE_URL}/admin/pets`, {
        withCredentials: true,
      });
      return response.data;
    } catch (error) {
  ...
  getPetById: async (petId: string): Promise<Pet> => {
    try {
      const response: AxiosResponse<Pet> = await axios.get(`${API_BASE_URL}/pets/${petId}`);
      return response.data;
    } catch (error) {
  ...
  uploadPetImages: async (petId: string, images: File[]): Promise<void> => {
    try {
  ...
  deletePetImages: async (petId: string, imagesToDelete: string[]): Promise<void> => {
    try {
      const response: AxiosResponse<void> = await axios.delete(`${API_BASE_URL}/pets/${petId}/images`, {
