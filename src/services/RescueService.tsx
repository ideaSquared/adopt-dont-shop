...
const RescueService = {
  fetchRatings: async (rescueId: string): Promise<any> => {
    try {
      const response = await axios.get(`${API_BASE_URL}/ratings/find-ratings/${rescueId}`, {
  ...
  createConversation: async (rescueId: string | null, petId: string, userId: string | null): Promise<any> => {
    try {
      const participants = [
  ...
  updateRescueProfile: async (rescueId: string, updates: any): Promise<any> => {
    try {
      const response = await axios.put(`${API_BASE_URL}/rescue/${rescueId}`, updates, {
        withCredentials: true,
      });
      return response.data;
    } catch (error) {
  ...
  submitReferenceNumberForVerification: async (
    ...
    try {
      ...
      console.error('Error submitting reference number for verification:', error);
      throw error;
    }
  },
  ...
  fetchAdminRescueDetails: async (rescueId: string): Promise<any> => {
    try {
      const response = await axios.get(`${API_BASE_URL}/admin/rescues/${rescueId}`, {
        withCredentials: true,
      });
      return response.data;
    } catch (error) {
  ...
  deleteStaffFromAdminRescue: async (rescueId: string, staffId: string): Promise<void> => {
    try {
      await axios.delete(`${API_BASE_URL}/admin/rescues/${rescueId}/staff/${staffId}`, {
        withCredentials: true,
      });
    } catch (error) {
