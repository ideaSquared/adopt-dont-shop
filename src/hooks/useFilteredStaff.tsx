...
export const useFilteredStaff = (rescueId: string | null) => {
  ...
  useEffect(() => {
    const filtered = staff.filter((staffMember: StaffMember) => {
      const nameEmailMatch =
        `${staffMember.firstName} ${staffMember.email}`.toLowerCase().includes(filterCriteria.nameEmail.toLowerCase());
      const permissionMatch =
        filterCriteria.permissions === 'all' || staffMember.permissions?.includes(filterCriteria.permissions);
      const verifiedMatch = !filterCriteria.verified || staffMember.verifiedByRescue;
  ...
  const handleFilterChange = (field: keyof FilterCriteria) => (
    ...
    const value = field === 'verified' && event.target instanceof HTMLInputElement
