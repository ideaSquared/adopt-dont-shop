...
import RescueService from '../services/RescueService'; // Adjust the path according to your project structure
import { Rescue } from '../types/rescue'; // Adjust the path according to your project structure
    ...
    const initFetch = async () => {
      try {
        ...
        if (error instanceof Error && 'response' in error && (error as any).response.status === 401) {
          navigate('/logout'); // Assuming you have a route to handle logout
        } else {
          setAlertInfo({
            type: 'danger',
