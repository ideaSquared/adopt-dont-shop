...
```typescript
new PetsService(config?: PetsServiceConfig)
```
...
- `options` (PetsServiceOptions): Operation options
...
function MyComponent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await petsService.exampleMethod({
...
app.get('/api/pets/example', async (req, res) => {
  try {
    const result = await petsService.exampleMethod(req.body);
    res.json(result);
  } catch (error) {
...
# Install dependencies
RUN cd lib.pets && npm ci && npm run build
...
├── src/
│   ├── services/
│   │   ├── pets-service.ts     # Main service implementation
│   │   └── __tests__/
│   │       └── pets-service.test.ts
│   ├── types/
...
```typescript
petsService.updateConfig({ debug: true });
```
