...
const StatusButton = styled(Button)<{ active: boolean }>`
  ...
  border-radius: 20px;
  border: 1px solid
    ${props => (props.active ? props.theme.colors.primary[500] : props.theme.colors.neutral[300])};
  ...
  &:hover {
    ...
    background: ${props =>
      props.active ? props.theme.colors.primary[600] : props.theme.colors.primary[50]};
...
const StatusCount = styled.span<{ active: boolean }>`
  ...
  background: ${props =>
    props.active ? 'rgba(255, 255, 255, 0.2)' : props.theme.colors.neutral[200]};
  border-radius: 12px;
  font-size: 0.75rem;
  font-weight: 500;
  color: ${props => (props.active ? 'white' : props.theme.text.secondary)};
    ...
    <StatusFilterContainer role="group" aria-label="Filter pets by status">
      {statusOptions.map(option => {
        ...
        return (
          ...
          >
            {option.label}
            <StatusCount active={isActive} aria-hidden="true">
              {count}
            </StatusCount>
          </StatusButton>
        );
