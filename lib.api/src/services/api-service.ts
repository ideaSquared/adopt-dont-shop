import { ApiServiceConfig, ApiServiceOptions, BaseResponse, ErrorResponse } from '../types';

/**
 * ApiService - Handles api operations
 */
  ...
  async exampleMethod(
    ...
    try {
      ...
      if (options.useCache && this.cache.has(cacheKey)) {
        if (this.config.debug) {
          console.log(`${ApiService.name} cache hit for key:`, cacheKey);
        }
      ...
      if (this.config.debug) {
        ...
        console.log(`${ApiService.name} exampleMethod completed in ${duration}ms`);
      }
      ...
      if (this.config.debug) {
        console.error(`${ApiService.name} exampleMethod failed:`, errorResponse);
      }
