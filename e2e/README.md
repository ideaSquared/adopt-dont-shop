...
>
> **Blocked on architecture, not yet un-parked** — `client/swipe-and-favourite.spec.ts` needs a favourites seed in the pets service (and its CSRF delete helper reworked to Bearer) — a rewrite plus a seed, not a config flip. `admin/2fa-enrollment` needs a full 2FA feature build (no `/auth/2fa/*` RPCs exist yet) (ADS-868). `admin/bulk-user-actions` (the `/admin/users` surface) and `rescue/custom-application-questions` (rescue-service question RPCs + `/rescues/:id/questions` routes) have since been built and un-parked.
>
