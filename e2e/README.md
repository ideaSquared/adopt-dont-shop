...
>
> **Blocked on architecture, not yet un-parked** — `client/swipe-and-favourite.spec.ts` is the last one: it needs a favourites seed in the pets service (and its CSRF delete helper reworked to Bearer) — a rewrite plus a seed, not a config flip. The other three feature specs — `admin/bulk-user-actions` (`/admin/users` surface), `rescue/custom-application-questions` (rescue question RPCs + routes) and `admin/2fa-enrollment` (full 2FA: proto + auth handlers + login enforcement + `/auth/2fa/*` routes + the lib.auth login prompt) — have since been built and un-parked.
>
