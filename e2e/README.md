...
>
> **Blocked on architecture, not yet un-parked** — `client/swipe-and-favourite.spec.ts` needs a favourites seed in the pets service (and its CSRF delete helper reworked to Bearer) — a rewrite plus a seed, not a config flip. `rescue/custom-application-questions`, `admin/bulk-user-actions`, `admin/2fa-enrollment` need new gateway routes built (ADS-868).
>
