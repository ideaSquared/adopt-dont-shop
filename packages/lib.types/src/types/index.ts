...
export type Permission =
  ...
  | 'emails.send'
  | 'emails.queue.manage'
  // Singular "email.*" — this is the literal the notifications
  // email-template admin RPCs actually gate on
  // (services/notifications/src/grpc/email-template-handlers.ts), per the
  // proto's own comment. Distinct from the "emails.*" (plural) literals
  // above; keep both rather than consolidating (ADS-1235).
  | 'email.templates.read'
  | 'email.templates.create'
  | 'email.templates.update'
  | 'email.templates.delete'
  | 'auth.privacy-prefs.read'
  | 'auth.privacy-prefs.read:any'
  | 'auth.privacy-prefs.update'
  | 'auth.privacy-prefs.update:any'
