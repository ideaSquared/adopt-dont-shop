...
Grafana is then available at <http://localhost:3030>. Sign in with `admin` / the `GF_SECURITY_ADMIN_PASSWORD` value from your `.env` (auto-generated by `pnpm bootstrap` / `pnpm secrets:generate`, alongside the other app secrets).
